...
for p in range(1, 16):
    ...
    if not res1.status_code == 200:
        print("http status: " + str(res1.status_code))
        time.sleep(10)
    j = res1.json()
    s1 = json.dumps(j)
...
for index in range(0, 300):
    ...
    if not res2.status_code == 200:
        print("http status: " + str(res2.status_code))
        time.sleep(10)
    sim = res2.json()
...
for i in range(len(listBoth)):
    print(str(listBoth[i][0]) + "," + str(listBoth[i][1]), file=open("movie_ID_sim_movie_ID.csv", "a+"))
